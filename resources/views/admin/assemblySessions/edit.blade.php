...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.assemblySession.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.assembly-sessions.update", [$assemblySession->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.assemblySession.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $assemblySession->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kla_number') ? 'is-invalid' : '' }}" type="number" name="kla_number" id="kla_number" value="{{ old('kla_number', $assemblySession->kla_number) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('session_number') ? 'is-invalid' : '' }}" type="number" name="session_number" id="session_number" value="{{ old('session_number', $assemblySession->session_number) }}" step="1">
