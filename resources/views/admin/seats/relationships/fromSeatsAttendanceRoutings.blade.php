    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-fromSeatsAttendanceRoutings">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <th>
                                {{ trans('cruds.attendanceRouting.fields.to_js_as_ss_employee') }}
                            </th>
                            <th>
                            ...
                            <th>
                                {{ trans('cruds.attendanceRouting.fields.to_seat') }}
                            </th>
                            <th>
                                {{ trans('cruds.seat.fields.slug') }}
                            </th>
                            <th>
                                {{ trans('cruds.attendanceRouting.fields.from_seats') }}
                            </th>
                            <th>
                                {{ trans('cruds.attendanceRouting.fields.from_employees') }}
                            </th>
                            <th>
                                &nbsp;
                            </th>
                    ...
                    <tbody>
                        @foreach($attendanceRoutings as $key => $attendanceRouting)
                            <tr data-entry-id="{{ $attendanceRouting->id }}">
                                ...
                                <td>
                                    {{ $attendanceRouting->to_js_as_ss_employee->name ?? '' }}
                                </td>
                                <td>
                                    {{ $attendanceRouting->to_js_as_ss_employee->pen ?? '' }}
                                </td>
                                <td>
                                    {{ $attendanceRouting->to_js_as_ss_employee->aadhaarid ?? '' }}
                                </td>
                                <td>
                                    {{ $attendanceRouting->to_seat->title ?? '' }}
                                </td>
                                <td>
                                    {{ $attendanceRouting->to_seat->slug ?? '' }}
                                </td>
                                <td>
                                    @foreach($attendanceRouting->from_seats as $key => $item)
                                        <span class="badge badge-info">{{ $item->title }}</span>
                                    @endforeach
                                </td>
                                <td>
                                    @foreach($attendanceRouting->from_employees as $key => $item)
...
@endcan
  ...
  let table = $('.datatable-fromSeatsAttendanceRoutings:not(.ajaxTable)').DataTable({ buttons: dtButtons })
