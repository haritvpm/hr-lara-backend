        ...
        <form method="POST" action="{{ route("admin.employees.update", [$employee->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $employee->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('ot_data_entry_by_admin') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="ot_data_entry_by_admin" id="ot_data_entry_by_admin" value="1" {{ $employee->ot_data_entry_by_admin || old('ot_data_entry_by_admin', 0) === 1 ? 'checked' : '' }}>
