@extends('layouts.admin')
@section('content')
@can('flexi_application_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.flexi-applications.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.flexiApplication.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.flexiApplication.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-FlexiApplication">
                <thead>
                    <tr>
                        <th width="10">

                        </th>
                        <th>
                            {{ trans('cruds.flexiApplication.fields.id') }}
                        </th>
                        <th>
                            {{ trans('cruds.flexiApplication.fields.employee') }}
                        </th>
                        <th>
                            {{ trans('cruds.flexiApplication.fields.aadhaarid') }}
                        </th>
                        <th>
                            {{ trans('cruds.flexiApplication.fields.flexi_minutes') }}
                        </th>
                        <th>
                            {{ trans('cruds.flexiApplication.fields.with_effect_from') }}
                        </th>
                        <th>
                            {{ trans('cruds.flexiApplication.fields.owner_seat') }}
                        </th>
                        <th>
                            {{ trans('cruds.flexiApplication.fields.approved_by') }}
                        </th>
                        <th>
                            {{ trans('cruds.flexiApplication.fields.approved_on') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($flexiApplications as $key => $flexiApplication)
                        <tr data-entry-id="{{ $flexiApplication->id }}">
                            <td>

                            </td>
                            <td>
                                {{ $flexiApplication->id ?? '' }}
                            </td>
                            <td>
                                {{ $flexiApplication->employee->name ?? '' }}
                            </td>
                            <td>
                                {{ $flexiApplication->aadhaarid ?? '' }}
                            </td>
                            <td>
                                {{ $flexiApplication->flexi_minutes ?? '' }}
                            </td>
                            <td>
                                {{ $flexiApplication->with_effect_from ?? '' }}
                            </td>
                            <td>
                                {{ $flexiApplication->owner_seat ?? '' }}
                            </td>
                            <td>
                                {{ $flexiApplication->approved_by ?? '' }}
                            </td>
                            <td>
                                {{ $flexiApplication->approved_on ?? '' }}
                            </td>
                            <td>
                                @can('flexi_application_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.flexi-applications.show', $flexiApplication->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('flexi_application_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.flexi-applications.edit', $flexiApplication->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('flexi_application_delete')
                                    <form action="{{ route('admin.flexi-applications.destroy', $flexiApplication->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                    </form>
                                @endcan

                            </td>

                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('flexi_application_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.flexi-applications.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  });
  let table = $('.datatable-FlexiApplication:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
})

</script>
@endsection