    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.punching-traces.update", [$punchingTrace->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('day_offset') ? 'is-invalid' : '' }}" type="number" name="day_offset" id="day_offset" value="{{ old('day_offset', $punchingTrace->day_offset) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('punching') ? 'is-invalid' : '' }}" name="punching_id" id="punching_id" required>
                    @foreach($punchings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('punching_id') ? old('punching_id') : $punchingTrace->punching->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
