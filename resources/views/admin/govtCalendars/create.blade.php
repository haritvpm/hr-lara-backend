...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.govt-calendars.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('attendance_trace_fetch_complete') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="attendance_trace_fetch_complete" value="0">
                    <input class="form-check-input" type="checkbox" name="attendance_trace_fetch_complete" id="attendance_trace_fetch_complete" value="1" {{ old('attendance_trace_fetch_complete', 0) == 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="attendance_trace_fetch_complete">{{ trans('cruds.govtCalendar.fields.attendance_trace_fetch_complete') }}</label>
                </div>
                @if($errors->has('attendance_trace_fetch_complete'))
                    <span class="text-danger">{{ $errors->first('attendance_trace_fetch_complete') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('day_type') ? 'is-invalid' : '' }}" name="day_type" id="day_type">
                    ...
                    @foreach(App\Models\GovtCalendar::DAY_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('day_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
